...
export default {
  ...
  methods: {
    fetchResources() {
      ...
      // IMPORTANT: Replace '/student/resources' with your actual API endpoint
      axios.get('/student/resources', { params }) 
        .then(response => {
           // Adjust according to your backend response structure
          this.resources = response.data.videos || response.data.resources || [];
          this.totalResources = response.data.totalItems || response.data.total || 0;
        })
        .catch(error => {
