      ...
      this.$refs.resourceForm.validate(async (valid) => {
        ...
        try {
          ...
          if (this.resourceForm.videoFile) {
            ...
            videoFormData.append('file', this.resourceForm.videoFile);
            
            // 使用直接上传接口
            const videoResponse = await axios.post('/api/upload/video/direct', videoFormData, {
            ...
            console.log('视频直接上传成功:', videoResponse.data);
            
            if (videoResponse.data.success) {
              this.resourceForm.videoUrl = videoResponse.data.data.url;
          ...
          if (this.resourceForm.thumbnailFile) {
            ...
            thumbnailFormData.append('file', this.resourceForm.thumbnailFile);
            
            const thumbnailResponse = await axios.post('/api/upload/image', thumbnailFormData, {
              onUploadProgress: (progressEvent) => {
                const percentCompleted = Math.round((progressEvent.loaded * 100) / progressEvent.total);
            ...
            console.log('缩略图上传成功:', thumbnailResponse.data);
            
            if (thumbnailResponse.data.success) {
              this.resourceForm.thumbnailUrl = thumbnailResponse.data.data.url;
            } else {
              throw new Error(thumbnailResponse.data.message || '缩略图上传失败');
            }
          }
          
          // 3. 判断是否需要保存到数据库
          const saveToDatabase = confirm('视频已上传成功！是否保存到数据库？');
          ...
          if (saveToDatabase) {
            ...
            // 添加已上传的URL
            formData.append('videoUrl', this.resourceForm.videoUrl);
            if (this.resourceForm.thumbnailUrl) {
              formData.append('thumbnailUrl', this.resourceForm.thumbnailUrl);
            }
            ...
            if (token) {
              ...
              console.log('保存视频使用认证Token:', token.substring(0, 10) + '...');
            } else {
              this.$message.error('未检测到认证信息，请确认您已登录后再尝试保存');
              this.saving = false;
              return;
            }
            
            if (this.editMode) {
              await axios.put(`/api/teachers/videos/${this.resourceId}`, formData, { headers });
              this.$message.success('视频资源更新成功');
            } else {
              const response = await axios.post('/api/teachers/videos', formData, { headers });
              console.log('保存响应:', response.data);
              this.$message.success('视频资源保存成功');
            }
            ...
            // 用户选择不保存到数据库，只是关闭对话框
            this.$message.info('视频已上传但未保存到数据库');
            this.resourceDialogVisible = false;
          }
          ...
          if (error.response && error.response.status === 401) {
            this.$message.error('认证失败，请确认您已登录且有权限上传视频');
            setTimeout(() => {
